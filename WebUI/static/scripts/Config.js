...
function addInput(divName) {
    ...
    newColCam.innerHTML = "<label>Camera Name: </label> <input type='text' id='CameraName" + counter + "' name='CameraName" + counter + "' placeholder='FrontDoor' required/>";

    document.getElementById('addedRow' + counter).appendChild(newColCam);

    newColIp.className = "col-lg-12";
    var id = "addedRow" + counter;
    newColIp.innerHTML = "<label>Camera Ip: </label> <input type='text' id='CameraIp" + counter + "' name='CameraIp" + counter + "' placeholder='0.0.0.0' required/> <input type='button' onclick='return deleteInput(" + '"' + id + '"' + ")' value='Remove' />";
